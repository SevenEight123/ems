			...
			if data.keystate == 1:  # Down events only
				...
				if(data.scancode == 28):
					...
					if (scan_code == "max" or scan_code == "julia") and (username is None):     
						...
						showText(0,10,"Hallo "+str(username))
						showText(0,20,"Bitte Einkaufsort")
						showText(0,30,"scannen")  
						...
						if (exists != 0):
							...
							if type(product) is dict:
								...
								cur.execute("INSERT INTO products(ean,name,detailname,vendor,maincat,subcat,descr,origin,price,pic) VALUES ( %s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(product_ean,product["name"],product["detailname"],product["vendor"],product["maincat"],product["subcat"],product["descr"],product["origin"],price,pic))
					...
					elif scan_code=="takepic":
						

						cur = db.cursor(mdb.cursors.DictCursor)

						cur.execute("SELECT COUNT(*) FROM pictures")
						pic_count = cur.fetchone()
						
						cur.close()

						picnumber = pic_count["COUNT(*)"]+1
						

						picpath = take_picture(False,picnumber)

						cur = db.cursor(mdb.cursors.DictCursor)

						cur.execute("INSERT INTO pictures(picpath) VALUES (%s)",(picpath,))
						...
						clearDisplay()
						showText(0,0,"Bild in DB eingetragen")
						time.sleep(2)
						clearDisplay()
